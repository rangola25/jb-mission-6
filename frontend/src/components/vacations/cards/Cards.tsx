...
export default function Vacations(): JSX.Element {
    ...
    (async () => {
        try {
            if (vacationsRedux.length === 0) {
                ...
                vacationsFromServer.sort((a, b) => new Date(a.startDate).getTime() - new Date(b.startDate).getTime())
                ...
                // Sort vacations by start date
                const sortedVacations = [...vacationsRedux].sort((a, b) => new Date(a.startDate).getTime() - new Date(b.startDate).getTime())
  ...
  useEffect(() => {
       ...
       if (filter === 'followed') {
          ...
          console.log('Filtered active vacations:', filteredVacations)
          if (filteredVacations?.length !== 0) {
              setVacations(filteredVacations)
          } else {
              setVacations([])  // No vacations found, set to empty array
